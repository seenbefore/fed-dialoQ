...
try {
    if (page.VueConfig) {
        const VueConfig = require(page.VueConfig)
        VUE_APP_ENTRY_CONFIG = VueConfig
    }
} catch (err) {
    console.warn(err)
}
...
const staticResourceMap = {
    ...
    devAndProd: {
        ...
        cdn: {
            css: [
                process.env.BASE_URL + 'static/assets/npm/element-ui/2.13.2/theme-chalk/index.css',
                //process.env.BASE_URL + 'static/assets/npm/element-ui/2.13.2/theme-custom/theme/index.css',
            ...
            js: [
                ...
                //     'static/assets/npm/echarts-liquidfill/echarts-liquidfill.min.js',
 ...
 */
module.exports = () => {
    if (typeof VUE_APP_ENTRY_CONFIG === 'function') {
        return VUE_APP_ENTRY_CONFIG()
    }
    ...
    return ({
        ...
        configureWebpack: config => {
            ...
            return {
                // 打包后将dist/static下的js文件删除注释
                plugins: [new OutputJSPlugin(process.env.outputDir)],
                //plugins: [],
            }
        },
