...
export class Dictionary<TDict extends DictionaryType<string, DictItemType>> {
    ...
    translate<T extends keyof TDict>(name: T, val: any) {
        ...
        let target = options.find(item => {
            const include = typeof item.value === 'number' ? item.value === val : item.value.split(',').includes(val?.toString() ?? '')
     ...
     * @returns {any} 字典选项
     */
    getOption<T extends keyof TDict, K extends TDict[T]['options'][number]['value'] | TDict[T]['options'][number]['enum'], R = TDict[T]['options'][number]>(name: T, val: K): R {
        const options = this.getOptions(name) || []
        let target = options.find(item => {
            const include = typeof item.value === 'number' ? item.value === val : item.value.split(',').includes(val?.toString() ?? '')
            return item.label == val || item.value == val || include
        ...
        function recursiveFormat(obj: any) {
            Object.keys(obj).forEach(key => {
                ...
                if (_dict[key]) {
                    ...
                    if (target) {
                        const label = target.tag !== undefined ? target.tag : target.label
                        obj[`${key}$`] = {
                            label,
                            class: target.class || '',
                            value: target.value,
