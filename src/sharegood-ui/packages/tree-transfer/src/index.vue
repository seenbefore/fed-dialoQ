    ...
    <div class="wl-transfer transfer" :style="{ width, height }">
        ...
        <div class="transfer-left">
            ...
            <div class="transfer-main">
                <slot name="from"></slot>
                <el-input v-if="filter" clearable size="small" class="filter-tree" :placeholder="placeholder" v-model="filterFrom"></el-input>
                <el-tree
                    ref="from-tree"
                    show-checkbox
        ...
        <div class="transfer-center address-list-center">
            <p class="transfer-center-item address-only-item">
                <el-button type="primary" @click="addressListTransfer()" icon="el-icon-arrow-right" class="address-first-btn" :disabled="from_disabled">添加</el-button>
            </p>
        </div>
        <!-- 右侧列表区 -->
        <div class="transfer-right">
            <div class="transfer-right-item transfer-right-only">
                ...
                <div class="transfer-main">
                    <slot name="to"></slot>
                    <el-input v-if="filter" clearable :placeholder="placeholder" v-model="filterListFirst" size="small" class="filter-tree"></el-input>
                    <ul class="address-list-ul">
                        <li class="address-list-li" v-for="item of addressee" :key="item[node_key]">
                            ...
                            <i class="address-list-del el-icon-delete" @click="clearList(item[node_key])"></i>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    methods: {
        ...
        addressListTransfer() {
            ...
            arrayDeWeighting = arrayCheckedNodes.filter(item => {
                if (!this.addressee.some(ite => ite[this.node_key] == item[this.node_key])) {
                    return item
                }
            })
        ...
        // 清理 右侧选中 数据
        clearList(id) {
            this.addressee = id == 'all' ? [] : this.addressee.filter(item => item[this.node_key] != id)
