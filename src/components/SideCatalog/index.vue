    ...
    <div class="side-catalog" :style="{ height }">
        ...
        <div class="side-catalog__list">
            <div>
                ...
                >
                    <!-- 每行插槽 -->
                    <slot name="row" :level="item.level" :isActive="active === item.ref" :title="item.title">
                        <!-- 每行icon插槽 -->
                        <slot name="default" :level="item.level" :isActive="active === item.ref">
                            <i
...
export default {
    ...
    methods: {
        ...
        activeIntoView(edge) {
            // 等active元素改变后
            this.$nextTick(() => {
                const activeEl = document.querySelector('.side-catalog__list-item--active')
                if (!activeEl) return
        ...
        topForDom() {
            ...
            // )
            const childrenList = Array.from(document.querySelector(this.container).querySelectorAll('*'))
