...
export const exData = Object.freeze({
    onBefore(config: ExAxiosRequestConfig) {
        ...
        const body = config.data || config.params || config.body || {}

        if (body.page || body.pageNum || body.length || body.pageSize) {
            body.length = body.pageSize || body.length || 10
            body.pageNum = body.page || body.pageNum || 1

            body.start = (body.pageNum - 1) * body.length
            delete body.page
